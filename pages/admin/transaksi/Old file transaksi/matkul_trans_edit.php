...
<?php include("../sidebaradmin.php")?>
        ...
        <div id="page-wrapper">
                ...
                <?php
$sql = mysql_query("SELECT MATAKULIAH.KODEMATKUL,MATAKULIAH.NAMAMATKUL,MATAKULIAH.KATAUP,
MATAKULIAH.SKS,MATAKULIAH.TAHUNKURIKULUM,MATAKULIAH.KODESEMESTER,MATAKULIAH.SEMESTER,
SEMESTER.NAMASEMESTER,MATAKULIAH.STATUSAKTIF ,MATAKULIAH.STATUSTAWAR FROM MATAKULIAH 
INNER JOIN SEMESTER ON SEMESTER.KODESEMESTER=MATAKULIAH.KODESEMESTER 
WHERE KODEMATKUL='$_GET[kodematkul]'") or die (mysql_error());

mysql_select_db($nama_db, $koneksi) or die("Gagal memilih database!");
$row=mysql_fetch_array($sql);
?>
<form method="post" action="matkul_trans_update.php" style="font-size:16px">
<div class="form-group">
<label>Kode Matkul</label>
<input class="form-control" type="text" name="kodematkul" value="<?php echo $row['KODEMATKUL']; ?>" maxlength="6" autofocus disabled/><br>
<input type="hidden" name="kodematkul" value="<?php echo $row['KODEMATKUL']; ?>" maxlength="6" autofocus/>
</div>

<div class="form-group">
<input type="hidden" name="namamatkul" value="<?php echo $row['NAMAMATKUL']; ?>"/>
<label>Nama Matkul</label>
<input class="form-control" type="text" name="namamatkul" value="<?php echo $row['NAMAMATKUL']; ?>" required/><br>
</div>

<div class="form-group">
<input type="hidden" name="sks" value="<?php echo $row['SKS']; ?>"/>
<label>SKS</label>
<input class="form-control" type="number" name="sks" value="<?php echo $row['SKS']; ?>" required/><br>
</div>
...
<input class="form-control" type="number" name="thnkuri" value="<?php echo $row['TAHUNKURIKULUM']; ?>" required/><br>
</div>
...
<select class="form-control" name="kodesemester" id="kodesemester" required>
     <option value="<?php echo $row['KODESEMESTER']; ?>" selected="selected">--<?php echo $row['NAMASEMESTER']; ?>--</option>
             <?php
        $querysemester = mysql_query("select kodesemester,namasemester from semester") or die (mysql_error());
...
<a href="matkul_trans_insertview.php"><button type="button" class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span> Batal</button></a></td>
</form>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
