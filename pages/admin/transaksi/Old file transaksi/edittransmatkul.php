...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
                ...
                <div class="col-lg-12">
                    <h1 class="page-header">UBAH DATA PENAWARAN MATA KULIAH</h1>
                </div>
...
<select class="form-control" name="nip" required>
     <option value="<?php echo $row['NIP']; ?>" selected="selected">--<?php echo $row['NAMAPEGAWAI']; ?>--</option>
        <?php
        $querypeg = mysql_query("select nip,namapegawai from pegawai where kodejabatan='jab002' group by nip") or die (mysql_error());
        //$satuan = $queryjabatan['namajabatan'];
        while($barispeg = mysql_fetch_array($querypeg)){
...
<select class="form-control" name="kodematkul" id="kodematkul" required>
     <option value="<?php echo $row['KODEMATKUL']; ?>" selected="selected">--<?php echo $row['NAMAMATKUL']; ?>--</option>
        <?php
        $querymatkul = mysql_query("select tawaranmatkul.kodematkul,matakuliah.namamatkul from matakuliah inner join tawaranmatkul on tawaranmatkul.kodematkul=matakuliah.kodematkul
		where tawaranmatkul.statustawaran='aktif' group by tawaranmatkul.kodematkul") or die (mysql_error());
        //$satuan = $queryjabatan['namajabatan'];
        while($barismatkul = mysql_fetch_array($querymatkul)){
            echo "<option value='".$barismatkul['kodematkul']."'>".$barismatkul['namamatkul']."</option>";
        }
        ?>
</select><br>
</div>
...
<select class="form-control" name="kodethnajaran" required>
     <option value="<?php echo $row['KODETHNAJARAN']; ?>" selected="selected">--<?php echo $row['TAHUNAJARAN']; ?>--</option>
        <?php
        $querymatkul = mysql_query("select tawaranmatkul.kodethnajaran,tahunajaran.tahunajaran from tahunajaran inner join tawaranmatkul on tawaranmatkul.kodethnajaran=tahunajaran.kodethnajaran
		where tawaranmatkul.statustawaran='aktif' group by tawaranmatkul.kodethnajaran") or die (mysql_error());
        //$satuan = $queryjabatan['namajabatan'];
        while($barismatkul = mysql_fetch_array($querymatkul)){
            echo "<option value='".$barismatkul['kodethnajaran']."'>".$barismatkul['tahunajaran']."</option>";
        }
        ?>
</select><br>
</div>
...
<select class="form-control" name="kodesemester" required>
     <option value="<?php echo $row['KODESEMESTER']; ?>" selected="selected">--<?php echo $row['NAMASEMESTER']; ?>--</option>
        <?php
        $querymatkul = mysql_query("select tawaranmatkul.kodesemester,semester.namasemester from semester inner join tawaranmatkul on tawaranmatkul.kodesemester=semester.kodesemester
		where tawaranmatkul.statustawaran='aktif' group by tawaranmatkul.kodesemester") or die (mysql_error());
        //$satuan = $queryjabatan['namajabatan'];
        while($barismatkul = mysql_fetch_array($querymatkul)){
            echo "<option value='".$barismatkul['kodesemester']."'>".$barismatkul['namasemester']."</option>";
        }
        ?>
</select><br>
</div>
...
<a href="insertview_trans_matkul.php"><button type="button" class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span> Batal</button></a></td>
</form>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
