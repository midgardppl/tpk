...
//tawaranmatkul.kodethnajaran=tahunajaran.kodethnajaran and tawaranmatkul.kodesemester=semester.kodesemester
    ...
    <select class="form-control dependant" name="ta" id="ta" required onchange="viewDt();">
     <option value="-" selected="selected" class="null">-- Semua --</option>
        <?php
        $querythn = mysqli_query($koneksi,"select * from tahunajaran") or die (mysqli_error($koneksi));
...
<select class="form-control dependant" name="hr" id="hr" required onchange="viewDt();">
     <option value="-" selected="selected" class="null">-- Semua --</option>
        <?php
        $querysem = mysqli_query($koneksi,"select * from pegawai where kodejabatan='jab002'") or die (mysql_error($koneksi));
        //$satuan = $queryjabatan['namajabatan'];
        while($barissem = mysqli_fetch_array($querysem)){
...
echo "</thead>";
echo "<tbody>";
$sql = mysqli_query($koneksi,"select dk.kodematkul, dt.koderuang, dt.kodehari, dt.kodejam, dt.kodejam2 from detailjadwal dt, detailkelas dk where dt.kodekelas=dk.kodekelas and dt.kodeprodi=dk.kodeprodi group by dt.iddetail") or die (mysqli_error($koneksi));
