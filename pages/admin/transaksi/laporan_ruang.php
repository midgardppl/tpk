...
var ttt = $('#hr').val();
	...
	$.ajax({
		url: "getRuang.php?t="+t+"&tt="+tt+"&ttt="+ttt,
...
<select class="form-control dependant" name="hr" id="hr" required onchange="viewDt();">
     <option value="-" selected="selected" class="null">-- Semua --</option>
        <?php
        $querysem = mysqli_query($koneksi,"select * from ruangan") or die (mysqli_error($koneksi));
        //$satuan = $queryjabatan['namajabatan'];
        while($barissem = mysqli_fetch_array($querysem)){
            echo "<option value='".$barissem[0]."'>".$barissem[2]."</option>";
        }
        ?>
</select><br>
</div>
...
?>
    <tr>
        ...
        <td><?php echo $row[4];?></td>
        		<td><?php if($row[4]=="MENUNGGU"){}else{ echo '<button type="submit" onclick="javascript: printExternal(\'print_jadwal.php?kd='.$row[0].'\')">Cetak</button>';}?></td>
